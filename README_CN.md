...
- [example](example)
  - 示例程序
- [kexample](kexample)
  - KMDF 内核使用示例，直接以内核工具链编译同一份 `cpueaxh` 核心源码
...
内核示例位于 [kexample/main.cpp](kexample/main.cpp)，展示了如何在 KMDF 非 PnP 驱动中运行 guest 模式示例，并直接以内核方式编译同一份核心源码，从而保持用户态与内核态共享同一个模拟器核心实现。
