...
int decode_cmpxchg_rm_index(CPU_CONTEXT* ctx, uint8_t modrm) {
...
uint64_t read_cmpxchg_reg_operand(CPU_CONTEXT* ctx, int reg_index, int operand_size) {
    switch (operand_size) {
...
void write_cmpxchg_reg_operand(CPU_CONTEXT* ctx, int reg_index, int operand_size, uint64_t value) {
    switch (operand_size) {
...
uint64_t read_cmpxchg_rm_operand(CPU_CONTEXT* ctx, uint8_t modrm, uint64_t mem_addr, int operand_size) {
    uint8_t mod = (modrm >> 6) & 0x03;
    int rm_index = decode_cmpxchg_rm_index(ctx, modrm);
    if (mod == 3) {
        return read_cmpxchg_reg_operand(ctx, rm_index, operand_size);
    }
...
void write_cmpxchg_rm_operand(CPU_CONTEXT* ctx, uint8_t modrm, uint64_t mem_addr, int operand_size, uint64_t value) {
    uint8_t mod = (modrm >> 6) & 0x03;
    int rm_index = decode_cmpxchg_rm_index(ctx, modrm);
    if (mod == 3) {
        write_cmpxchg_reg_operand(ctx, rm_index, operand_size, value);
        return;
    }
...
uint64_t read_cmpxchg_accumulator(CPU_CONTEXT* ctx, int operand_size) {
    switch (operand_size) {
...
void write_cmpxchg_accumulator(CPU_CONTEXT* ctx, int operand_size, uint64_t value) {
    switch (operand_size) {
    case 8:  set_reg8(ctx, REG_RAX, (uint8_t)value); break;
...
void cmpxchg_update_flags(CPU_CONTEXT* ctx, int operand_size, uint64_t accumulator_value, uint64_t dst_value) {
    switch (operand_size) {
    case 8: {
        uint8_t lhs = (uint8_t)accumulator_value;
        uint8_t rhs = (uint8_t)dst_value;
        uint16_t result = (uint16_t)lhs - (uint16_t)rhs;
        update_flags_sub8(ctx, lhs, rhs, result);
        break;
    }
    case 16: {
        uint16_t lhs = (uint16_t)accumulator_value;
        uint16_t rhs = (uint16_t)dst_value;
        uint32_t result = (uint32_t)lhs - (uint32_t)rhs;
        update_flags_sub16(ctx, lhs, rhs, result);
        break;
    }
    case 32: {
        uint32_t lhs = (uint32_t)accumulator_value;
        uint32_t rhs = (uint32_t)dst_value;
        uint64_t result = (uint64_t)lhs - (uint64_t)rhs;
        update_flags_sub32(ctx, lhs, rhs, result);
        break;
    }
    case 64: {
        uint64_t result = accumulator_value - dst_value;
        update_flags_sub64(ctx, accumulator_value, dst_value, result);
        break;
    }
    default:
        raise_ud_ctx(ctx);
...
void cmpxchg_rm_reg(CPU_CONTEXT* ctx, uint8_t modrm, uint64_t mem_addr, int operand_size, bool has_lock_prefix) {
    ...
    if (accumulator_value == dst_value) {
        write_cmpxchg_rm_operand(ctx, modrm, mem_addr, operand_size, src_value);
    }
    else {
        write_cmpxchg_accumulator(ctx, operand_size, dst_value);
    }
...
DecodedInstruction decode_cmpxchg_instruction(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
