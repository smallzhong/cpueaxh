...
uint64_t read_shl_rm_operand(CPU_CONTEXT* ctx, uint8_t modrm, uint64_t mem_addr, int operand_size) {
    uint8_t mod = (modrm >> 6) & 0x03;
    int rm = decode_shl_rm_index(ctx, modrm);
...
void write_shl_rm_operand(CPU_CONTEXT* ctx, uint8_t modrm, uint64_t mem_addr, int operand_size, uint64_t value) {
    uint8_t mod = (modrm >> 6) & 0x03;
    int rm = decode_shl_rm_index(ctx, modrm);
...
uint8_t get_shl_count_mask(int operand_size) {
    return operand_size == 64 ? 0x3F : 0x1F;
}

uint8_t decode_shl_count(CPU_CONTEXT* ctx, const DecodedInstruction* inst) {
    switch (inst->opcode) {
    case 0xD0:
    case 0xD1:
        return 1;
...
void update_flags_shl(CPU_CONTEXT* ctx, uint64_t result, int operand_size, unsigned int count, bool carry_out) {
    if (count == 0) {
        return;
    }
...
void shl_rm(CPU_CONTEXT* ctx, uint8_t modrm, uint8_t sib, int32_t disp, uint64_t mem_addr, int operand_size, uint8_t raw_count) {
    (void)sib;
    (void)disp;
    ...
    for (unsigned int i = 0; i < count; i++) {
        carry_out = (result & sign_mask) != 0;
        result = (result << 1) & mask;
    }

    write_shl_rm_operand(ctx, modrm, mem_addr, operand_size, result);
    update_flags_shl(ctx, result, operand_size, count, carry_out);
}
...
DecodedInstruction decode_shl_instruction(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
