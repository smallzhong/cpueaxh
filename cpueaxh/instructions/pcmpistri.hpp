...
static uint64_t pcmpistri_segment_base(CPU_CONTEXT* ctx, int segment_index) {
    SegmentRegister* seg = get_segment_register(ctx, segment_index);
    return seg ? seg->descriptor.base : 0;
}

static int decode_pcmpistri_xmm_reg_index(CPU_CONTEXT* ctx, uint8_t modrm) {
...
static int decode_pcmpistri_xmm_rm_index(CPU_CONTEXT* ctx, uint8_t modrm) {
    int rm = modrm & 0x07;
    if (ctx->rex_b) {
        rm |= 0x08;
    }
...
static void decode_modrm_pcmpistri(CPU_CONTEXT* ctx, DecodedInstruction* inst, uint8_t* code, size_t code_size, size_t* offset) {
    if (*offset >= code_size) {
        raise_gp_ctx(ctx, 0);
    }
...
static XMMRegister read_pcmpistri_source(CPU_CONTEXT* ctx, const DecodedInstruction* inst) {
    if (((inst->modrm >> 6) & 0x03) == 3) {
        return get_xmm128(ctx, decode_pcmpistri_xmm_rm_index(ctx, inst->modrm));
    }
    return read_xmm_memory(ctx, inst->mem_address);
}

bool is_pcmpistri_instruction(const uint8_t* code, int len, int prefix_len) {
    if (prefix_len + 3 >= len) {
        return false;
    }
    return code[prefix_len] == 0x0F && code[prefix_len + 1] == 0x3A && code[prefix_len + 2] == 0x63;
}

DecodedInstruction decode_pcmpistri_instruction(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
    ...
    while (offset < code_size) {
        ...
        if (prefix == 0x66 || prefix == 0xF2 || prefix == 0xF3) {
            ...
            int seg_override = decode_pcmpistri_segment_override(prefix);
            if (seg_override >= 0) {
    ...
    decode_modrm_pcmpistri(ctx, &inst, code, code_size, &offset);
    ...
    if (inst.has_modrm) {
        ...
        if (mod != 3) {
            ...
            inst.mem_address += pcmpistri_segment_base(ctx, effective_segment);
        }
    }
...
void execute_pcmpistri(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
    DecodedInstruction inst = decode_pcmpistri_instruction(ctx, code, code_size);
    int lhs_index = decode_pcmpistri_xmm_reg_index(ctx, inst.modrm);
    __m128i lhs = avx_xmm_to_m128i(get_xmm128(ctx, lhs_index));
    __m128i rhs = avx_xmm_to_m128i(read_pcmpistri_source(ctx, &inst));
