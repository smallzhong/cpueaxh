...
void write_setcc_rm_operand(CPU_CONTEXT* ctx, uint8_t modrm, uint64_t mem_addr, uint8_t value) {
    uint8_t mod = (modrm >> 6) & 0x03;
    int rm = decode_setcc_rm_index(ctx, modrm);
...
DecodedInstruction decode_setcc_instruction(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
    ...
    decode_modrm_setcc(ctx, &inst, code, code_size, &offset, has_lock_prefix);
