...
uint64_t read_rol_rm_operand(CPU_CONTEXT* ctx, uint8_t modrm, uint64_t mem_addr, int operand_size) {
    uint8_t mod = (modrm >> 6) & 0x03;
    int rm = decode_rol_rm_index(ctx, modrm);
...
void write_rol_rm_operand(CPU_CONTEXT* ctx, uint8_t modrm, uint64_t mem_addr, int operand_size, uint64_t value) {
    uint8_t mod = (modrm >> 6) & 0x03;
    int rm = decode_rol_rm_index(ctx, modrm);
...
uint8_t get_rol_count_mask(int operand_size) {
...
void update_flags_rol(CPU_CONTEXT* ctx, uint64_t result, int operand_size, unsigned int count, bool carry_out) {
    if (count == 0) {
        return;
    }
...
void rol_rm(CPU_CONTEXT* ctx, uint8_t modrm, uint8_t sib, int32_t disp, uint64_t mem_addr, int operand_size, uint8_t raw_count) {
    (void)sib;
    (void)disp;
    ...
    update_flags_rol(ctx, result, operand_size, count, carry_out);
}
...
DecodedInstruction decode_rol_instruction(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
...
void execute_rol(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
    ...
    rol_rm(ctx, inst.modrm, inst.sib, inst.displacement, inst.mem_address, inst.operand_size, count);
}
