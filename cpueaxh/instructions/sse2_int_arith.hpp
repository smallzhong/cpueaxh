...
int decode_sse2_int_arith_xmm_rm_index(CPU_CONTEXT* ctx, uint8_t modrm) {
    int rm = modrm & 0x07;
    if (ctx->rex_b) {
        rm |= 0x08;
    }
...
void decode_modrm_sse2_int_arith(CPU_CONTEXT* ctx, DecodedInstruction* inst, uint8_t* code, size_t code_size, size_t* offset, bool has_lock_prefix) {
    if (*offset >= code_size) {
        raise_gp_ctx(ctx, 0);
    }
...
DecodedInstruction decode_sse2_int_arith_instruction(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size, uint8_t* mandatory_prefix) {
    ...
    *mandatory_prefix = 0;
    ...
    if (inst.opcode != 0xD4 && inst.opcode != 0xD5 && inst.opcode != 0xD8 && inst.opcode != 0xD9 &&
        inst.opcode != 0xDA && inst.opcode != 0xDC && inst.opcode != 0xDD && inst.opcode != 0xDE &&
        inst.opcode != 0xE0 && inst.opcode != 0xE3 && inst.opcode != 0xE4 && inst.opcode != 0xE5 &&
        inst.opcode != 0xE8 && inst.opcode != 0xE9 && inst.opcode != 0xEA && inst.opcode != 0xEC &&
        inst.opcode != 0xED && inst.opcode != 0xEE && inst.opcode != 0xF4 && inst.opcode != 0xF5 &&
        inst.opcode != 0xF6 && inst.opcode != 0xF8 && inst.opcode != 0xF9 && inst.opcode != 0xFA &&
        inst.opcode != 0xFB && inst.opcode != 0xFC && inst.opcode != 0xFD && inst.opcode != 0xFE) {
        raise_ud_ctx(ctx);
    }
    ...
    decode_modrm_sse2_int_arith(ctx, &inst, code, code_size, &offset, has_lock_prefix);
...
void sse2_int_arith_xmm_to_bytes(XMMRegister value, uint8_t bytes[16]) {
...
XMMRegister sse2_int_arith_bytes_to_xmm(const uint8_t bytes[16]) {
    XMMRegister value = {};
    for (int index = 0; index < 8; index++) {
...
uint16_t sse2_int_arith_read_u16(const uint8_t bytes[16], int index) {
    int offset = index * 2;
    return (uint16_t)bytes[offset] | ((uint16_t)bytes[offset + 1] << 8);
}

uint32_t sse2_int_arith_read_u32(const uint8_t bytes[16], int index) {
    int offset = index * 4;
...
uint64_t sse2_int_arith_read_u64(const uint8_t bytes[16], int index) {
    int offset = index * 8;
    uint64_t value = 0;
...
int16_t sse2_int_arith_read_i16(const uint8_t bytes[16], int index) {
    return (int16_t)sse2_int_arith_read_u16(bytes, index);
}

void sse2_int_arith_write_u16(uint8_t bytes[16], int index, uint16_t value) {
    int offset = index * 2;
    bytes[offset] = (uint8_t)(value & 0xFFU);
    bytes[offset + 1] = (uint8_t)((value >> 8) & 0xFFU);
}

void sse2_int_arith_write_u32(uint8_t bytes[16], int index, uint32_t value) {
    int offset = index * 4;
    bytes[offset] = (uint8_t)(value & 0xFFU);
    bytes[offset + 1] = (uint8_t)((value >> 8) & 0xFFU);
...
void sse2_int_arith_write_u64(uint8_t bytes[16], int index, uint64_t value) {
    int offset = index * 8;
    for (int byte_index = 0; byte_index < 8; byte_index++) {
...
int8_t sse2_int_arith_saturate_i16_to_i8(int16_t value) {
    if (value > 127) {
        return 127;
    }
    if (value < -128) {
...
uint8_t sse2_int_arith_saturate_i16_to_u8(int16_t value) {
    if (value < 0) {
        return 0;
    }
    if (value > 255) {
...
int16_t sse2_int_arith_saturate_i32_to_i16(int32_t value) {
    if (value > 32767) {
        return 32767;
    }
    if (value < -32768) {
...
uint8_t sse2_int_arith_average_u8(uint8_t lhs, uint8_t rhs) {
    return (uint8_t)(((uint16_t)lhs + (uint16_t)rhs + 1U) >> 1);
}

uint16_t sse2_int_arith_average_u16(uint16_t lhs, uint16_t rhs) {
    return (uint16_t)(((uint32_t)lhs + (uint32_t)rhs + 1U) >> 1);
}

XMMRegister read_sse2_int_arith_source_operand(CPU_CONTEXT* ctx, const DecodedInstruction* inst) {
    uint8_t mod = (inst->modrm >> 6) & 0x03;
    if (mod == 3) {
        return get_xmm128(ctx, decode_sse2_int_arith_xmm_rm_index(ctx, inst->modrm));
    }

    return read_xmm_memory(ctx, inst->mem_address);
}

void execute_sse2_int_arith(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
    uint8_t mandatory_prefix = 0;
    DecodedInstruction inst = decode_sse2_int_arith_instruction(ctx, code, code_size, &mandatory_prefix);
    int dest = decode_sse2_int_arith_xmm_reg_index(ctx, inst.modrm);
    uint8_t lhs_bytes[16] = {};
    uint8_t rhs_bytes[16] = {};
    uint8_t result_bytes[16] = {};

    sse2_int_arith_xmm_to_bytes(get_xmm128(ctx, dest), lhs_bytes);
    sse2_int_arith_xmm_to_bytes(read_sse2_int_arith_source_operand(ctx, &inst), rhs_bytes);
    ...
    case 0xEA:
        for (int index = 0; index < 8; index++) {
            int16_t lhs = sse2_int_arith_read_i16(lhs_bytes, index);
            int16_t rhs = sse2_int_arith_read_i16(rhs_bytes, index);
            sse2_int_arith_write_u16(result_bytes, index, (uint16_t)((lhs < rhs) ? lhs : rhs));
        }
        break;
    case 0xEE:
        for (int index = 0; index < 8; index++) {
    ...
    case 0xE5:
        for (int index = 0; index < 8; index++) {
            ...
            sse2_int_arith_write_u16(result_bytes, index, (uint16_t)((product >> 16) & 0xFFFF));
        }
        break;
    case 0xF6:
    ...
    set_xmm128(ctx, dest, sse2_int_arith_bytes_to_xmm(result_bytes));
}