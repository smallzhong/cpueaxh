...
int decode_movsxd_reg_index(CPU_CONTEXT* ctx, uint8_t modrm) {
...
uint32_t read_movsxd_rm_operand(CPU_CONTEXT* ctx, uint8_t modrm, uint64_t mem_addr) {
    uint8_t mod = (modrm >> 6) & 0x03;
    int rm = decode_movsxd_rm_index(ctx, modrm);
...
void write_movsxd_reg_operand(CPU_CONTEXT* ctx, uint8_t modrm, int64_t value) {
    int reg = decode_movsxd_reg_index(ctx, modrm);
    set_reg64(ctx, reg, (uint64_t)value);
}

void movsxd_r64_rm32(CPU_CONTEXT* ctx, uint8_t modrm, uint8_t sib, int32_t disp, uint64_t mem_addr) {
    (void)sib;
    (void)disp;
    int32_t value = (int32_t)read_movsxd_rm_operand(ctx, modrm, mem_addr);
    write_movsxd_reg_operand(ctx, modrm, value);
}
...
DecodedInstruction decode_movsxd_instruction(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
    ...
    decode_modrm_movsxd(ctx, &inst, code, code_size, &offset, has_lock_prefix);
