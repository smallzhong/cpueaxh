...
static int decode_roundss_xmm_rm_index(CPU_CONTEXT* ctx, uint8_t modrm) {
    int rm = modrm & 0x07;
    if (ctx->rex_b) {
        rm |= 0x08;
    }
...
static unsigned int roundss_host_rounding_mode(uint32_t mxcsr) {
...
static __m128 apply_roundss_intrinsic(__m128 destination, __m128 source, uint8_t imm8, uint32_t mxcsr) {
    if ((imm8 & 0x04) != 0) {
        unsigned int old_mode = _MM_GET_ROUNDING_MODE();
        _MM_SET_ROUNDING_MODE(roundss_host_rounding_mode(mxcsr));
        __m128 result = _mm_round_ss(destination, source, _MM_FROUND_CUR_DIRECTION | _MM_FROUND_NO_EXC);
...
static XMMRegister apply_roundss128(XMMRegister destination, uint32_t source_bits, uint8_t imm8, uint32_t mxcsr) {
    XMMRegister source_scalar = {};
    set_xmm_lane_bits(&source_scalar, 0, source_bits);
    return sse_math_m128_to_xmm(apply_roundss_intrinsic(sse_math_xmm_to_m128(destination), sse_math_xmm_to_m128(source_scalar), imm8, mxcsr));
}

inline bool is_roundss_instruction(const uint8_t* code, int len, int prefix_len) {
    if (!code || prefix_len + 4 >= len) {
        return false;
    }
    return code[prefix_len] == 0x0F && code[prefix_len + 1] == 0x3A && code[prefix_len + 2] == 0x0A;
}

inline DecodedInstruction decode_roundss_instruction(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
    ...
    decode_modrm_roundss(ctx, &inst, code, code_size, &offset);
...
inline void execute_roundss(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
    DecodedInstruction inst = decode_roundss_instruction(ctx, code, code_size);
    if (cpu_has_exception(ctx)) {
        return;
    }

    int dest = decode_roundss_xmm_reg_index(ctx, inst.modrm);
    XMMRegister destination = get_xmm128(ctx, dest);
    uint32_t source_bits = 0;

    if (((inst.modrm >> 6) & 0x03) == 0x03) {
        source_bits = get_xmm_lane_bits(get_xmm128(ctx, decode_roundss_xmm_rm_index(ctx, inst.modrm)), 0);
    }
    else {
        source_bits = read_memory_dword(ctx, inst.mem_address);
    }
    ...
    set_xmm128(ctx, dest, apply_roundss128(destination, source_bits, (uint8_t)inst.immediate, ctx->mxcsr));
}
